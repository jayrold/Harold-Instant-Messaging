...
class Tim extends CI_Controller {
	...
	function logout(){
		...
		setcookie("ttm_remember_me", "", time()-3600, '/','');
		//redirect('tim/index');
	...
	public function index(){
		//echo sha1("kdjskjds");
		$data['login_flag'] = $this->login_flag;
		$this->load->view('view_tim_login', $data);
	}
	...
	public function process_login(){
		...
		$this->form_validation->set_message('required', '<b>%s</b> is required!');
		$this->form_validation->set_message('valid_email', '<b>%s</b> must contain a valid email address.');
		$data['msg'] = "";
		$data['error_flag'] = "no";
		$data['username'] = "";
		$data['key'] = "";
		if($this->form_validation->run() == TRUE){
			//$data['msg'] =  $email." ".$password." ".$remember_me;
			
			$this->db->where('email', $email);
			$this->db->where('password', sha1($password));
			$user = $this->db->get('tim_chat_users')->row();
			
			if($user){
		
				//Check if rememberme is valid
				if($remember_me){

					//Generate a token for cookie
					$this->load->helper('string');
					$token_cookie = random_string('unique');

					//Set cookie in database
					$this->db->update('tim_chat_users',
						array('token' => $token_cookie),
						array('username' => $user->username)
					);
				
					$cookie_value = $user->username.','.$token_cookie;
				
					//Set cookie in user's computer
					setcookie("ttm_remember_me", $cookie_value, time()+1209600, '/','');
				}
				...
				$data['error_flag'] = "yes";
				$data['msg'] =  "<p>Invalid Email/Password!</p>";
			}	
		}
		else{
			$data['error_flag'] = "yes";
	...
	function ajax_jchat(){
		...
		foreach($chat_his as $c){
			...
			if($c->from == $to){
				$msg .='<li style=\'background-color: #F2CFF7; padding-left: 10px;\'>
	...
	function ajax_jchat_update($to = "", $from = ""){
		...
		*/
		...
		foreach($chat_his as $c){
			...
			if($c->from == $to){
				$data['msg'] .='<li style=\'background-color: #F2CFF7; padding-left: 10px;\'>
					<em>'.$c->from.' says:</em><br>
					'.$c->message.'<br>
					<span>'.date('d M Y h:i:s a', mysql_to_unix($c->time_added)).'</span>
				</li>';
